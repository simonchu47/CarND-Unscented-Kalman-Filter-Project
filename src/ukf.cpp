...
UKF::UKF() {
  ...
  */
  ...
  for (int index = 1; index < n_sigma; index++) {
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  int n_sigma = 2*n_aug_ + 1;
  MatrixXd Xsig_aug_ = MatrixXd(n_aug_, n_sigma); 
  ...
  for (int col_num = 0; col_num < n_sigma; col_num++) {
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  Zsig = Xsig_pred_.block(0, 0, 2, n_sigma); 
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  //update state mean and covariance matrix
  VectorXd z_dif = z - z_pred;
  tools.KeepDiffInTwoPi(z_dif(1), PI);
  x_ = x_ + K*z_dif;
  P_ = P_ - K*S*K.transpose();
